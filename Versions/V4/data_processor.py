...
class AzureLogProcessor:
    ...
    def analyze_privileged_sessions(self) -> Dict[str, Any]:
        """Analisa sessões privilegiadas para detectar padrões suspeitos."""
        if self.logs_df is None:
            return {
                "suspicious_sessions": [],
        ...
        suspicious_sessions = []
        multiple_ip_sessions = defaultdict(set)
        concurrent_sessions = []
        off_hours_sessions = []

        # Filtra logs relevantes
        auth_logs = self.logs_df[
            self.logs_df['operation_name'].str.contains('login|signin|auth', 
                                                       case=False, 
                                                       na=False)
        ].copy()
        ...
        # Organiza por usuário e timestamp
        auth_logs.sort_values('timestamp', inplace=True)

        for user, user_logs in auth_logs.groupby('user_principal_name'):
            # Analisa múltiplos IPs
            user_ips = set(user_logs['ip_address'].dropna())
            if len(user_ips) > 3:  # Threshold para múltiplos IPs
                multiple_ip_sessions[user].update(user_ips)

            # Analisa sessões concorrentes
            for i in range(len(user_logs) - 1):
                current = user_logs.iloc[i]
                next_login = user_logs.iloc[i + 1]
                
                if (next_login['timestamp'] - current['timestamp']) < timedelta(minutes=5):
                    if current['ip_address'] != next_login['ip_address']:
                        concurrent_sessions.append({
                            'user': user,
                            'first_ip': current['ip_address'],
                            'second_ip': next_login['ip_address'],
                            'time_diff': str(next_login['timestamp'] - current['timestamp'])
                        })

            # Analisa logins fora do horário comercial
            for _, session in user_logs.iterrows():
                hour = session['timestamp'].hour
                if hour < 6 or hour > 22:  # Fora do horário comercial estendido
        ...
        return {
            "suspicious_sessions": [
                ...
                for user, ips in multiple_ip_sessions.items()
            ],
            "multiple_ip_sessions": [
                {
                    'user': user,
            ...
            "off_hours_sessions": off_hours_sessions
        }
            
        # Cria DataFrame vazio para atribuições de roles se necessário
        if self.role_assignments_df is None:
            self.role_assignments_df = pd.DataFrame(columns=['user_principal_name', 'role_name', 'scope'])
            
        # Extrai atribuições de roles dos logs
        role_logs = self.logs_df[
            self.logs_df['operation_name'].str.contains('role', case=False, na=False) |
            self.logs_df['operation_name'].str.contains('permission', case=False, na=False)
        ] if 'operation_name' in self.logs_df.columns else pd.DataFrame()
        
        if not role_logs.empty and 'role_name' in role_logs.columns:
            new_assignments = role_logs[['user_principal_name', 'role_name', 'scope']].dropna()
            self.role_assignments_df = pd.concat([self.role_assignments_df, new_assignments], ignore_index=True)
    ...
    def analyze_orphaned_accounts(self) -> Dict[str, Any]:
        ...
        if 'user_principal_name' in self.logs_df.columns and 'timestamp' in self.logs_df.columns:
            columns_to_agg = {'timestamp': ['count', 'max']}
            if 'operation_name' in self.logs_df.columns:
                columns_to_agg['operation_name'] = 'nunique'
            
            user_activity = self.logs_df.groupby('user_principal_name').agg(columns_to_agg).reset_index()
            
            # Configura nomes das colunas com base nas agregações disponíveis
            new_columns = ['user']
            if 'timestamp' in columns_to_agg:
                new_columns.extend(['activity_count', 'last_activity'])
            if 'operation_name' in columns_to_agg:
                new_columns.append('unique_operations')
            
            user_activity.columns = new_columns
            ...
            for _, row in low_activity_users.iterrows():
                ...
                if 'unique_operations' in row.index:
                    account_data['unique_operations'] = row['unique_operations']
                    
                orphaned_accounts.append(account_data)
    ...
    def analyze_service_principal_risks(self) -> Dict[str, Any]:
        ...
        if 'user_principal_name' not in self.logs_df.columns:
            return {"sp_risks": [], "total_risks": 0}
            
        sp_mask = (self.logs_df['user_principal_name'].str.contains('ServicePrincipal', na=False) |
                 ~self.logs_df['user_principal_name'].str.contains('@', na=False))
        sp_activities = self.logs_df[sp_mask]

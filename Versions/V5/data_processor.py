...
class AzureLogProcessor:
    ...
    def load_logs_from_dict(self, logs_data: List[Dict[str, Any]]) -> pd.DataFrame:
        """Carrega e normaliza logs de uma lista de dicionários (dados do Blob Storage)."""
        try:
            if not logs_data:
                raise ValueError("Lista de logs está vazia")
            ...
            raise ValueError(f"Erro ao processar dados do dicionário: {e}")

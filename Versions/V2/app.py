...
st.set_page_config(
    page_title="Azure Governance Analytics", 
    page_icon="🛡️", 
    layout="wide",
...
def render_action_center(enhanced_result: EnhancedAIAnalysisResult):
    ...
    with col3:
        assignee_filter = st.selectbox(
            "Responsável:",
            ["Todos", "IT Security", "Identity Team", "Compliance", "Management"]
        )
    
    # Simulação de lista de ações
    actions_data = []
    for i, finding in enumerate(enhanced_result.findings[:10], 1):
        priority_level = finding.risk_level.value
        action_id = f"ACT-{i:03d}"
        
        # Simulação de status e responsável
        import random
        statuses = ["Pendente", "Em Andamento", "Concluída"]
        assignees = ["IT Security", "Identity Team", "Compliance", "Management"]
        ...
        time_estimates = {
            "Critical": "24h", "High": "3 dias", "Medium": "1 semana", "Low": "2 semanas"
        }
...
def export_report_to_json(analysis_result, governance_summary):
    ...
    report_data = {
        ...
        "report_metadata": {
            "version": "2.0",
            "tool": "Azure Governance Analytics",
...
def main():
    ...
    try:
        ...
        st.markdown('<h1 class="main-header">🛡️ Azure Governance Analytics</h1>', unsafe_allow_html=True)
        ...
        with st.sidebar:
            ...
            if not openai_configured:
                with st.expander("🔧 Configurar Azure OpenAI"):
                    ...
                    st.code("""
# No config.py, defina:
config.openai_api_key = "sua_chave_api_aqui"
                    """)
        ...
        # Conteúdo principal
        st.markdown("---")
        st.subheader("1️⃣ Upload e Processamento de Logs")
        ...
        if uploaded_file:
            try:
                with st.spinner("🔄 Processando arquivo de logs..."):
        ...
        <div style='text-align: center; color: #666; padding: 2rem;'>
            <p>🛡️ <strong>Azure Governance Analytics</strong> - Ferramenta de Análise de Governança para Microsoft Azure</p>
